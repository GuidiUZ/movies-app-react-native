...
const DownLoadScreen = () => {
  ...
  const { onGetMovies, onDeleteMovies } = useMoviesStorage();


  const loadMovies = async () => {
    try {
      const moviesResponse = await onGetMovies();
  ...
  const onRefresh = async () => {
    setRefreshing(true);
    try {
      const movieRefresh = await onGetMovies();
  ...
  const handleDeleteMovie = async (itemTitle) => {
    try {
      await onDeleteMovies(itemTitle);
      await loadMovies();
      return Promise.resolve();
    } catch (error) {
      return Promise.reject(error);
